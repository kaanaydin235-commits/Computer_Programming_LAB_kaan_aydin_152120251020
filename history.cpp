...
public:
    ...
    void display() const {
        ...
        for (Page* p = head; p; p = p->next, ++idx) {
            // simple non-iomanip formatting: index. url | title | hh:mm:ss
...
// helper to get current local time (hour,min,sec)
...
int main() {
    ...
    // destructor will clear, but call explicitly for clarity
    // hist.clear();
    return 0;
}
